...
class TestNextModeNoEvent:
    ...
    def test_tuesday_evening_no_event_next_mode_is_current(self):
        """Tuesday 18:15, no event: no mode change in 2-day window → shows current mode."""
        hass = make_mock_hass()
        entry = make_mock_entry()
        hass.states.get.return_value = make_calendar_state(state="off")
        coordinator = HomeShiftCoordinator(hass, entry)

        # Tuesday March 10 18:15 — next midnights are Wed and Thu (both weekdays),
        # mode stays Travail → next_mode shows current mode, next_mode_at is None.
